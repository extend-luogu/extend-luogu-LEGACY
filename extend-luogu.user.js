...
mod.reg("benben", "@/", () => {
    ...
    function check (lv)
    {
        ...
        if (3<=lv && lv<=5)
        {
            _color=check_color[2]
        }
        if (6<=lv && lv<=7)
        {
            _color=check_color[0]
        }
        if (8<=lv && lv<=10)
        {
            _color=check_color[1]
        }
        if (lv>=3)
        {
            return `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="${ _color}" style="margin-bottom: -3px;">
